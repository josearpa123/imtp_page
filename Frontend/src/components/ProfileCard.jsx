...
function ProfileCardComponent({
  ...
  const tiltEngine = useMemo(() => {
    ...
    const setVarsFromXY = (x, y) => {
      ...
      // ✅ Tilt correcto (X depende de Y, Y depende de X) + amplitud tipo demo
      const properties = {
        '--pointer-x': `${percentX}%`,
        '--pointer-y': `${percentY}%`,
        ...
        '--rotate-x': `${round(-(centerY / 6))}deg`,
        '--rotate-y': `${round(centerX / 7)}deg`
      }
  ...
  const handlePointerEnter = useCallback(
    (event) => {
      ...
      // ✅ Estado coherente con CSS (wrap + shell)
